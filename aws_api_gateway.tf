...
resource "aws_api_gateway_resource" "electrs_rpc_proxy" {
   rest_api_id = aws_api_gateway_rest_api.main.id
   parent_id   = aws_api_gateway_resource.electrs_rpc.id
   path_part   = "{proxy+}"
}

resource "aws_api_gateway_method" "electrs_rpc_proxy" {
  rest_api_id   = aws_api_gateway_rest_api.main.id
  resource_id   = aws_api_gateway_resource.electrs_rpc_proxy.id
  http_method   = "ANY"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "electrs_rpc" {
  rest_api_id = aws_api_gateway_rest_api.main.id
  resource_id = aws_api_gateway_resource.electrs_rpc_proxy.id
  http_method = aws_api_gateway_method.electrs_rpc_proxy.http_method
